...
namespace MLS_Digital_MGM_API.Controllers
{
    ...
    public class DepartmentsController : Controller
    {
        ...
        public async Task<IActionResult> GetDepartments(int pageNumber = 1, int pageSize = 10)
        {
            try
            {
                ...
                if (dataTableParams.LoadFromRequest(_httpContextAccessor))
                {
                    ...
                    var totalRecords = await _repositoryManager.DepartmentRepository.CountAsync(pagingParameters);
        ...
        [HttpGet("count")]
        public async Task<IActionResult> DepartmentCountries()
        {
            try
            {
                // Get the count of countries from the data layer
                var count = await _repositoryManager.DepartmentRepository.GetDepartmentsCountAsync();
