...
ALTER TABLE `LicenseApplications` ADD `CertificateOfAdmission` tinyint(1) NOT NULL DEFAULT FALSE;

ALTER TABLE `LicenseApplications` ADD `ExplanationForNotSubmittingCertificateOfAdmission` varchar(250) NULL;

INSERT INTO `__EFMigrationsHistory` (`MigrationId`, `ProductVersion`)
VALUES ('20240620122345_adding certificate of admission and explanation for not submitting to license applications', '8.0.6');

COMMIT;

START TRANSACTION;

ALTER TABLE `CPDTrainings` MODIFY `RegistrationDueDate` datetime(6) NOT NULL DEFAULT '0001-01-01 00:00:00.000000';

CREATE TABLE `InvoiceRequestTypes` (
    `Id` int NOT NULL AUTO_INCREMENT,
    `Name` varchar(100) NOT NULL,
    `CreatedDate` datetime(6) NOT NULL,
    `Status` longtext NOT NULL,
...
CREATE TABLE `InvoiceRequests` (
    `Id` int NOT NULL AUTO_INCREMENT,
    `InvoiceRequestTypeId` int NOT NULL,
    `CreatedById` varchar(200) NULL,
    `CreatedDate` datetime(6) NOT NULL,
    `Status` longtext NOT NULL,
    `UpdatedDate` datetime(6) NOT NULL,
    `DeletedDate` datetime(6) NULL,
    PRIMARY KEY (`Id`),
    CONSTRAINT `FK_InvoiceRequests_InvoiceRequestTypes_InvoiceRequestTypeId` FOREIGN KEY (`InvoiceRequestTypeId`) REFERENCES `InvoiceRequestTypes` (`Id`) ON DELETE CASCADE,
...
CREATE INDEX `IX_InvoiceRequests_InvoiceRequestTypeId` ON `InvoiceRequests` (`InvoiceRequestTypeId`);

INSERT INTO `__EFMigrationsHistory` (`MigrationId`, `ProductVersion`)
VALUES ('20240625133337_added invoice request table', '8.0.6');

COMMIT;

START TRANSACTION;

ALTER TABLE `InvoiceRequests` ADD `Amount` double NOT NULL DEFAULT 0.0;

ALTER TABLE `InvoiceRequests` ADD `CustomerId` longtext NOT NULL;

INSERT INTO `__EFMigrationsHistory` (`MigrationId`, `ProductVersion`)
VALUES ('20240625140723_added invoice request table adding fees', '8.0.6');

COMMIT;

START TRANSACTION;

DROP TABLE `InvoiceRequestTypes`;

ALTER TABLE InvoiceRequests RENAME COLUMN InvoiceRequestTypeId TO YearOfOperationId;

ALTER TABLE `InvoiceRequests` RENAME INDEX `IX_InvoiceRequests_InvoiceRequestTypeId` TO `IX_InvoiceRequests_YearOfOperationId`;

ALTER TABLE `InvoiceRequests` MODIFY `Status` varchar(100) NOT NULL;

ALTER TABLE `InvoiceRequests` ADD `ReferencedEntityId` longtext NOT NULL;

ALTER TABLE `InvoiceRequests` ADD `ReferencedEntityType` longtext NOT NULL;

ALTER TABLE `InvoiceRequests` ADD CONSTRAINT `FK_InvoiceRequests_YearOfOperations_YearOfOperationId` FOREIGN KEY (`YearOfOperationId`) REFERENCES `YearOfOperations` (`Id`) ON DELETE CASCADE;

INSERT INTO `__EFMigrationsHistory` (`MigrationId`, `ProductVersion`)
VALUES ('20240626131858_removed invoice request type table', '8.0.6');

COMMIT;

START TRANSACTION;

ALTER TABLE `Members` ADD `CustomerId` varchar(250) NULL;

ALTER TABLE `Firms` ADD `CustomerId` varchar(250) NULL;
...
ALTER TABLE `Firms` ADD CONSTRAINT `FK_Firms_QBCustomers_CustomerId` FOREIGN KEY (`CustomerId`) REFERENCES `QBCustomers` (`Id`);

ALTER TABLE `Members` ADD CONSTRAINT `FK_Members_QBCustomers_CustomerId` FOREIGN KEY (`CustomerId`) REFERENCES `QBCustomers` (`Id`);

INSERT INTO `__EFMigrationsHistory` (`MigrationId`, `ProductVersion`)
VALUES ('20240628082601_added quickbooks tables', '8.0.6');

COMMIT;

START TRANSACTION;

ALTER TABLE `InvoiceRequests` MODIFY `CustomerId` varchar(250) NOT NULL;

ALTER TABLE `InvoiceRequests` ADD `Description` varchar(250) NOT NULL DEFAULT '';

ALTER TABLE `InvoiceRequests` ADD `QBInvoiceId` varchar(250) NULL;
...
ALTER TABLE `InvoiceRequests` ADD CONSTRAINT `FK_InvoiceRequests_QBCustomers_CustomerId` FOREIGN KEY (`CustomerId`) REFERENCES `QBCustomers` (`Id`) ON DELETE CASCADE;

ALTER TABLE `InvoiceRequests` ADD CONSTRAINT `FK_InvoiceRequests_QBInvoices_QBInvoiceId` FOREIGN KEY (`QBInvoiceId`) REFERENCES `QBInvoices` (`Id`);

INSERT INTO `__EFMigrationsHistory` (`MigrationId`, `ProductVersion`)
VALUES ('20240629165321_added description to invoice request', '8.0.6');
...
INSERT INTO `__EFMigrationsHistory` (`MigrationId`, `ProductVersion`)
VALUES ('20240630203939_added customer reference to firms', '8.0.6');
